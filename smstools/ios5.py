...
class IOS5:
    ...
    def parse_cursor(self, cursor):
        ...
        for row in query:
            if row[0]:
                txt = core.Text( num=row[1], date=int((row[3] + 978307200)*1000), incoming=(row[5]==0), body=row[4])
            else:
                from_me = row[6] & 0x01
                txt = core.Text( num=row[2], date=int(row[3]*1000), incoming=(from_me==1), body=row[4])
            if not txt.num:
                txt.num = "unknown"
